...
impl RigidBody2D {
    ...
    fn create_physics_object(world: &mut World<CustomUserData>, shape: ColliderType, body_type: &BodyType, position: Vec2/*, angle: f32*/) -> (BodyHandle, Uuid) {
        ...
        let body = match shape {
            ColliderType::Circle(radius) => {
                ...
                let body = world.create_body_with(&b_def, Some(uuid));
    ...
    pub fn new_circle_body(world: &mut World<CustomUserData>, body_type: &BodyType, position: Vec2, radius: f32) -> (RigidBody2D, Uuid) {
        let (body, uuid) = RigidBody2D::create_physics_object(world, Circle(radius), body_type, position);
        (RigidBody2D {
    ...
    pub fn new_box_body(world: &mut World<CustomUserData>, body_type: &BodyType, position: Vec2, scale: Vec2) -> (RigidBody2D, Uuid) {
        let (body, uuid) = RigidBody2D::create_physics_object(world, Box(scale), body_type, position);
